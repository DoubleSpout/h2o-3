...
/*    double[][] qmatT = transpose(qmat);
    double[][] newPenalty = ArrayUtils.multArrArr(ArrayUtils.multArrArr(penaltyMat, qmat), qmatT);
    double[][] multPenalty = ArrayUtils.multArrArr(ArrayUtils.multArrArr(qmat, qmatT), newPenalty);
    penaltyMat = add(subtract(penaltyMat,add(newPenalty, transpose(newPenalty))),multPenalty);*/
    return penaltyMat;
  }
}
