...
public class GAM extends ModelBuilder<GAMModel, GAMModel.GAMParameters, GAMModel.GAMModelOutput> {
  ...
  private class GAMDriver extends Driver {
    ...
    double[][][] _zTransposeCS; // store for each thin plate smoother to remove optimization constraint
    ...
    Frame adaptTrain() {
      ...
      _zTranspose = GamUtils.allocate3DArray(numGamFrame, _parms, firstOneLess);  // for centering for all type, top:cubic spline, 
      _penalty_mat = _parms._savePenaltyMat?GamUtils.allocate3DArray(numGamFrame, _parms, sameOrig):null; // bottom:tp
      ...
      _gamColMeans = new double[numGamFrame][]; // means of centeredGamCols
    ...
    public class CubicSplineSmoother extends RecursiveAction {
      ...
      protected void compute() {
        ...
        if (_savePenaltyMat) { // only save this for debugging
          copy2DArray(genOneGamCol._penaltyMat, _penalty_mat[_gamColIndex]); // copy penalty matrix
        ...
        copy2DArray(transformedPenalty, _penalty_mat_center[_gamColIndex]);
    ...
    void addGAM2Train() {
      ...
      for (int index = 0; index < numGamFrame; index++) { // generate smoothers/splines
        final Frame predictVec = prepareGamVec(index, _parms, _parms._train.get());  // extract predictors from training frame
        final int numKnots = _parms._num_knots_sorted[index];
        final int numKnotsM1 = numKnots - 1;
        if (_parms._bs_sorted[index] == 0) {// single predictor smoothers
